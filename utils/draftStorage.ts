...
export class DraftStorage {
  static async saveDraft(
    ...
    try {
      ...
      if (!thumbnailUri && segments.length > 0 && segments[0].uri) {
        console.log(
          `[DraftStorage] Generating thumbnail for draft: ${targetId}`
        );
  ...
  static async getLastModifiedDraft(mode?: DraftMode): Promise<Draft | null> {
    try {
      ...
      const mostRecent = filteredDrafts.reduce((latest, current) =>
        current.lastModified.getTime() > latest.lastModified.getTime()
          ? current
          : latest
      );
      ...
      if (mostRecent) {
        ...
        if (validatedDraft) {
          console.log(
            `[DraftStorage] Found last modified draft: ${validatedDraft.id} (${validatedDraft.segments.length} segments, mode: ${validatedDraft.mode})`
          );
          return validatedDraft;
        } else {
          console.log(
            `[DraftStorage] Last modified draft ${mostRecent.id} is corrupted, trying next most recent`
          );
          // Try to find the next most recent valid draft
          const remainingDrafts = filteredDrafts.filter(
  ...
  static async getDraftById(
    ...
    try {
      ...
      if (draft && mode && draft.mode !== mode) {
        console.log(
          `[DraftStorage] Draft ${id} mode mismatch: expected ${mode}, got ${draft.mode}`
        );
        return null;
      }
      ...
      if (draft) {
        ...
        if (validSegments.length === 0 && draft.segments.length > 0) {
          console.warn(
            `[DraftStorage] Draft ${id} has no valid segments, removing from storage`
          );
        ...
        if (validSegments.length !== draft.segments.length) {
          console.log(
            `[DraftStorage] Updating draft ${id} with ${
              validSegments.length
            } valid segments (removed ${
        ...
        console.log(
          `[DraftStorage] Retrieved draft: ${id} (${draft.segments.length} segments, mode: ${draft.mode})`
        );
      }
      return draft || null;
    } catch (error) {
  ...
  static async deleteDraft(
    ...
    try {
      ...
      console.log(
        `[DraftStorage] Deleted draft: ${id} (files ${
          options?.keepFiles ? "kept" : "deleted"
        })`
      );
    } catch (error) {
