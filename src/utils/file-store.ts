...
/**
 * The removed segment-upload staging sibling's path (`….upload.mp4`). Nothing creates these
 * anymore — kept only so `deleteSegmentFile` sweeps siblings left behind by older installs.
 */
...
export function deleteSegmentFile(relPath: string): void {
  ...
  // Sweep any historical staging sibling with its source — every lifecycle op
  // (delete/re-edit/reset) routes through here, so a leftover can never outlive the clip.
