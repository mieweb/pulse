...
const LARGE_UPLOAD_DRAFT_ID = 'dev-seed-large-upload';
...
// --- Large-upload draft -----------------------------------------------------------------------
// Dev sample 5: 20× the 24s recorder-match clip (4.9 MB) → one shared signature → the lossless
// passthrough join, so the merge stays fast while the video comes to ~98 MB (~8 min). Big enough
// that an upload runs long enough to test backgrounding, airplane mode and cancel mid-transfer.
// Reuses a clip the seed already bundles, so it adds nothing to the app bundle.
const LARGE_UPLOAD_MODULES: readonly number[] = Array.from(
  { length: 20 },
  () => FIXTURES[0].module,
);
...
/** Dev sample 5 — ~98 MB video for testing uploads mid-transfer (fast passthrough join). */
export function seedLargeUpload(): Promise<string | undefined> {
  return seedModules(LARGE_UPLOAD_DRAFT_ID, 'Dev sample 5 (large upload)', LARGE_UPLOAD_MODULES);
}
...
export async function clearDrafts(): Promise<void> {
  ...
  deleteDraftDir(LARGE_UPLOAD_DRAFT_ID);
}
