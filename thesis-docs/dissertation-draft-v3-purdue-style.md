...
I am deeply grateful to my advisor and committee members for their guidance, feedback, and support throughout this thesis journey. Their mentorship helped shape this work from an initial platform idea into a research-grounded systems dissertation.

I am also thankful to colleagues, friends, and collaborators who provided technical discussions, critical questions, and encouragement during implementation and evaluation.

Most importantly, I thank my family for their patience, encouragement, and constant belief in me.
...
**RQ5 (Optional human evidence).** _If IRB-approved:_ What **minimal** user-facing evidence (e.g., task-based usability or short survey) can **complement**—not substitute for—the design rationale and technical results?
...
- **Optional user validity (if applicable).** IRB compliance and small-_n_ limitations.
...
This chapter reports current thesis evidence status and the results framework that will hold final measured outputs.

### 8.1 Upload and Resilience

Initial validation indicates that the selected architecture is structurally prepared for reliable upload behavior because resumable ingest and explicit finalize boundaries are implemented and traceable to design rationale decisions in Chapter 5.

Final measured reporting to insert:

- Upload success rate across test conditions.
- Resume completion rate after interrupted sessions.
- Finalize latency distribution.

### 8.2 Processing and Queue Behavior

The queue-based transcode pipeline is implemented and instrumented, enabling reproducible measurement of processing behavior under varying load.

Final measured reporting to insert:

- Queue wait time and transcode duration percentiles.
- Time-to-playable distribution.
- Backlog response under worker replication.

### 8.3 Security and Playback

Signed URL and gateway policy controls are in place, creating a concrete basis for security-correctness and playback-path validation.

Final measured reporting to insert:

- Token acceptance-rejection matrix (valid, expired, tampered, wrong path).
- Playback startup latency by rendition/path.
- Access-control enforcement checks for restricted routes.

### 8.4 Resource and Operational Observations

Observability integration (metrics and logs) supports resource-level and operational diagnostics needed for systems-grade evaluation.

Final measured reporting to insert:

- CPU and memory profiles across API, worker, and gateway components.
- Error-rate and retry patterns during stress conditions.
- Operational bottlenecks observed in end-to-end runs.

### 8.5 Mapping Results to Research Questions

Current consolidated readiness from the thesis master compilation indicates:

- Methodology, survey instruments, and metric dictionary are high readiness.
- Literature support is triangulated across verified sources, standards references, and targeted Consensus findings.
- The principal remaining gap is final packaging of analyzable empirical exports.

Practical readiness estimate:

- Method and literature readiness: approximately 90%+.
- Final analyzable dataset packaging readiness: moderate.
- Overall readiness toward a defense-quality results chapter: approximately 85-90%.

Remaining outputs required before final lock:

1. Frozen usage events export for the thesis study window.
2. Baseline, midpoint, and endline survey response exports.
3. Computed SUS score table.
4. De-identified merged dataset linking usage and survey signals.
5. Interview transcript and coded-theme summary (if collected).

RQ coverage status at this stage:

- RQ1 and RQ2: addressed through architecture and design-rationale traceability.
- RQ3 and RQ4: methodologically ready, awaiting final measured inserts.
- RQ5: conditional on IRB-approved data collection.
...
Alahmari, S., & Renaud, K. (2020). Implement a model for describing and maximising security knowledge sharing. _15th International Conference for Internet Technology and Secured Transactions (ICITST)_.
...
Google. (2016). _Site reliability engineering: How Google runs production systems_. O'Reilly Media.

Hardt, D. (Ed.). (2012). _RFC 6749: The OAuth 2.0 authorization framework_. IETF. https://www.rfc-editor.org/rfc/rfc6749

Hevner, A. R., March, S. T., Park, J., & Ram, S. (2004). Design science in information systems research. _MIS Quarterly_, _28_(1), 75–105. https://doi.org/10.2307/25148625

Johnson, D. (2013). Mobile support in CSCW applications and groupware development frameworks. _International Journal of Interactive Mobile Technologies_, _7_(2), 54–62. https://doi.org/10.3991/ijim.v7i2.2469

Kruchten, P. (1995). The 4+1 view model of architecture. _IEEE Software_, _12_(6), 42–50. https://doi.org/10.1109/52.469759

Kumar, A., et al. (2024). Video-based approaches in health education: A systematic review and meta-analysis. _Scientific Reports_. https://doi.org/10.1038/s41598-024-73671-7

Lodderstedt, T., Bradley, J., Labunets, A., & Fett, D. (2025). _RFC 9700: Best current practice for OAuth 2.0 security_ (BCP 240). IETF. https://www.rfc-editor.org/rfc/rfc9700

Marron, J. (2024). _NIST SP 800-66 Rev. 2: Implementing the Health Insurance Portability and Accountability Act (HIPAA) Security Rule: A cybersecurity resource guide_. National Institute of Standards and Technology. https://doi.org/10.6028/NIST.SP.800-66r2

Mayer, R. E. (2005). Cognitive theory of multimedia learning. In R. E. Mayer (Ed.), _The Cambridge handbook of multimedia learning_ (pp. 31–48). Cambridge University Press.

Mayer, R. E. (2009). _Multimedia learning_ (2nd ed.). Cambridge University Press.

Nonaka, I. (1994). A dynamic theory of organizational knowledge creation. _Organization Science_, _5_(1), 14–37. https://doi.org/10.1287/orsc.5.1.14

Pantos, R., & May, W. (2017). _RFC 8216: HTTP live streaming_. IETF. https://www.rfc-editor.org/rfc/rfc8216

Prometheus Authors. (n.d.). _Prometheus documentation: Introduction to Prometheus_. https://prometheus.io/docs/introduction/overview/

Rajendran, P. T., Creusy, K., & Garnes, V. (2024). Shorts on the rise: Assessing the effects of YouTube Shorts on long-form video content. _arXiv_. https://arxiv.org/abs/2402.18208

Sandoval-Castaneda, M., Russell, B., Sivic, J., Shakhnarovich, G., & Caba Heilbron, F. (2025). EditDuet: A multi-agent system for video non-linear editing. _arXiv_. https://arxiv.org/abs/2509.10761

tus.io contributors. (2016-present). _Resumable upload protocol 1.0.0_. https://tus.io/protocols/resumable-upload.html

U.S. Department of Health and Human Services. (n.d.). _Summary of the HIPAA Security Rule_. https://www.hhs.gov/hipaa/for-professionals/security/laws-regulations/index.html
...
_End of draft structure. Expand Chapter 6 with your repository-specific paths and endpoint names as in prior versions; expand Chapter 8 with measured results when available._
